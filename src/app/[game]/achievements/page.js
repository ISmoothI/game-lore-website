...
export default function Achievements({ params }) {
    ...
    useEffect(() => {
        ...
        async function loadTrophies() {
            try {
                const res = await fetch(`/data/${game}/trophies.json`);
                const data = await res.json();

                setGameTitle(data.title);
                setTrophies(data.achievements);
                originalTrophiesRef.current = data.achievements;
                setTotalTrophies(data.achievements.length);
                setIncompleteTrophies(data.achievements.length);
            } catch (error) {
                setTrophies({error: "Game not found"});
            }
        }
